...
var NormalRow = React.createClass({
    pathForHamilton: '../hamilton/',

    answerTag: function() {
        if (this.props.answer === true) {
            var answerLink = this.pathForHamilton + 'answer.html' + '?id=p' + this.props.id;
            return (
    ...
    render: function() {
        var gameLink = this.pathForHamilton + 'game.html' + '?id=p' + this.props.id;
        var consoleLink = this.pathForHamilton + 'console.html' + '?id=p' + this.props.id;
        return (
            <tr>
                <td>{this.props.id}</td>
        ...
        return (
            <div>
                ...
                <table className="table">
                    ...
                    {data.map(
                        function(row) {
                            return <NormalRow key={row.id} id={row.id} name={row.name} N={row.N} M={row.M} answer={row.answer} />
                        }
                    )}
                    </tbody>
                </table>
...
export default function() {
    ReactDOM.render(
        <MainTable url="../../json/main/p1.json" />,
